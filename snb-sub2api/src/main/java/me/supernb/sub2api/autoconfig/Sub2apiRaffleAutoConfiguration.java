...
import me.supernb.sub2api.raffle.JdbcRaffleGateReadModel;
...
/// 发布会门槛只读读模型装配:不配 `sub2api.read-datasource.url` 就完全不装配。
...
public class Sub2apiRaffleAutoConfiguration {

    /// 门槛读模型 Bean:只读 DataSource 在本方法内现场构建,不对外暴露。
    @Bean
    @ConditionalOnMissingBean
    public RaffleGateReadModel raffleGateReadModel(Sub2apiProperties props) {
        ...
        DataSource ds = DataSourceBuilder.create()
                ...
                .build();
        return new JdbcRaffleGateReadModel(new JdbcTemplate(ds));
    }
}
