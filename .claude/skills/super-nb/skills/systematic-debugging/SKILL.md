...
6. **必要时深入取证**
   ...
   仍找不到根因时，再做更深入的证据收集：
   - 有 traceId/请求 ID 时，按它串联跨层、跨请求的日志，还原完整调用链
   - 不确定框架、依赖库（Spring Boot、JPA、Hibernate 等）的实际行为时，用 context7 查权威文档，或联网搜索确认——不要凭记忆假设
   - 必要时直接读框架或依赖库源码定位，而不是停留在猜测
...
1. **创建失败的测试用例**
   ...
   - 没有测试框架就写一次性测试脚本
   - 修复前必须先有测试
   - 使用 `super-nb:test-driven-development` 技能来编写规范的失败测试
...
**相关技能：**
- **super-nb:test-driven-development** - 用于创建失败测试用例（第四阶段，第 1 步）
- **super-nb:verification-before-completion** - 在宣称成功之前验证修复确实有效
