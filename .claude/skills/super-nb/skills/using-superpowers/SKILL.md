...
super-nb 技能覆盖默认系统提示行为，但**用户指令始终具有最高优先级**：

1. **用户的明确指令**（CLAUDE.md、AGENTS.md、直接请求）——最高优先级
2. **super-nb 技能** ——在冲突处覆盖默认系统行为
3. **默认系统提示** ——最低优先级
...
super-nb plugin 只针对 Claude Code 与 Codex 两个 AI 编码工具发行。
...
**注：** 流程图中"为每个条目创建 TodoWrite 待办"适用于 **skill 内部的检查清单**（如 brainstorming 的流程步骤、writing-plans 的自检步骤）。**执行 plan 任务的进度追踪不用 TodoWrite**——plan 是 Markdown 文件，进度通过勾选 `- [ ]` / `- [x]` 复选框跟踪（阻塞就行内标注），详见 `super-nb:executing-plans` 与 `super-nb:subagent-driven-development`。两条路径不冲突：前者是 skill 流程的临时步骤跟踪，后者是 plan 任务的持久进度跟踪（plan.md 复选框为单一真源）。
