...
✅ $ ./gradlew :snb-activity:snb-activity-app:test --tests "PerformDrawHandlerTest"
   ...
   PerformDrawHandlerTest > should_reject_when_no_active_campaign() PASSED
   PerformDrawHandlerTest > should_draw_available_slot() PASSED
...
```
✅ 真正的红-绿循环（以 PerformDrawCommand 紧凑构造器校验为例）：

   1. 先写测试（针对"campaignId 必填"这条不变式）：
      @Test
      void should_reject_missing_campaign() {
        assertThatThrownBy(() -> PerformDrawCommand.of(0L, 123L))
            .isInstanceOf(IllegalArgumentException.class);
      }

   2. 运行 → 必须红（构造器还没加校验）：
      $ ./gradlew :snb-activity:snb-activity-app:test --tests "PerformDrawCommandTest.should_reject_missing_campaign"
      > FAILED: expected IllegalArgumentException

   3. 加最小实现（紧凑构造器）：
      public PerformDrawCommand {
        if (campaignId <= 0) throw new IllegalArgumentException("campaignId 必填");
      }
