...
```bash
./gradlew test --no-daemon       # 单元测试 + 各模块 :test 内的 Testcontainers 集成测试
# 完成定义（编译 + 全部测试 + ArchUnit 六边形门禁）：
...
- **super-nb:brainstorming**（阶段 4）- 设计通过且需要实现时必需
- **super-nb:subagent-driven-development** - 执行任何任务前必需
- **super-nb:executing-plans** - 执行任何任务前必需
- 任何需要隔离工作区的技能

**配合使用：**

- **super-nb:finishing-a-development-branch** - 工作完成后清理时必需
