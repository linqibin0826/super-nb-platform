...
8. **是否严格按模板？**
   - 文档结构是否完整？（`# 标题` + 头部块 → 若干 `### 任务 N` → 末尾 `## 实施笔记`；注意"范围检查/文件结构/自检"是 writing-plans 的流程步骤，不会出现在 plan.md 正文里，别照它们核对标题）
   - 头部五项是否齐全、无占位符残留？
   - 全局约束是否已清晰列出，且各任务未与其矛盾？
   - 每个任务是否有唯一的 `### 任务 N` 标题？
   - 每个任务的步骤是否都用 `- [ ]` 复选框列出（初始态未勾选）？
   - 计划末尾是否已预留 `## 实施笔记` 章节（即使当前为空，供实施期追加偏离记录）？
