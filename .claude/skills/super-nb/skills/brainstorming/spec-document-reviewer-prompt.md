...
**调度时机：** 规格文档写入 <git-root>/docs/specs/ 之后
...
8. **文档格式是否规整？**
   - 是否残留占位符文本（`TODO`、`TBD`、`FILL`、`[占位]` 等未替换的模板标记）？
   - 全文内部是否一致（术语、字段命名、编号前后不矛盾）？
   - 范围是否收敛在单一 feature/子系统内，没有夹带不相关内容？
   - 是否存在模糊到可能被两种不同方式实现的表述（对上文"清晰度"一栏的最终扫一遍）？
