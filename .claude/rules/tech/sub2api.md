...
- **controller 鉴权**：方法参数 `@CurrentUser UserProfile user`（Authorization → introspect → active 终端用户 → 否则 401）
- **infra 薄适配**：注入 starter 读模型/客户端，把上游 DTO 映射为本上下文 app DTO（样板：`RechargeQueryAdapter`）
