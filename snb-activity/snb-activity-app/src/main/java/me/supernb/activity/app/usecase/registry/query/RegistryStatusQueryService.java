...
/// 活动中心注册表状态聚合(公开只读,spec 2026-07-12 §6):五个已知活动位各自的运行状态。
/// 字段白名单只有 id/kind/status/时间窗——绝不携带人数/名单/金额/身份,加字段须先改 spec。
/// lottery/raffle 无当前期时条目缺席(前端对缺席 id 静默无徽章);qq 窗口由配置推导;
/// leaderboard/checkin 常驻恒 running(checkin 2026-07-28 补齐,runbook 32 遗留待办——
/// registry.json 的 checkin 条目此前因 id 对不上一直没有状态徽章)。
/// 查询代价=两次索引点查+纯内存,照 leaderboard/pool 先例不加缓存。
@Service
public class RegistryStatusQueryService {
    ...
    public List<RegistryEntryStatus> status() {
        ...
        out.add(new RegistryEntryStatus("checkin", "evergreen", "running", null, null));
