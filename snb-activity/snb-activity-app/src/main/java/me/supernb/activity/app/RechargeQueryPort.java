...
public interface RechargeQueryPort {
    ...
    List<ActivityDto.LeaderEntry> leaderboard(Instant start, Instant end, int limit);

    List<ActivityDto.RechargeEntry> recentRecharges(Instant start, Instant end, int limit);

    /// 批量取脱敏邮箱(role=user);找不到的 id 不在返回 map。
    Map<Long, String> maskedEmailsByIds(Collection<Long> ids);

    /// 批量取兑换码状态;找不到的 code 不在返回 map。
    Map<String, ActivityDto.CodeStatus> codeStatuses(Collection<String> codes);
}
